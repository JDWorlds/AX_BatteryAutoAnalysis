...
interface MessageItem {
  id: string;
  role: "user" | "assistant";
  content: string; // HTML-safe string coming from backend (newlines already converted)
}
...
interface ExperimentInfo {
  ...
  total_cycles: number | undefined; // kept for backward compatibility; mirrored in scheduling
  safety_constraints: SafetyConstraints;
  scheduling: SchedulingPlan;
  thermal: ThermalControl;
  termination?: {
    capacity_fade_threshold_pct?: number; // e.g., stop when Qd <= 80% of initial
...
export default function RecipewithLLM() {
  ...
  // -------- Controlled form state (no querySelector!) --------
  ...
  async function handleSendMessage(text: string) {
    ...
    const payload = {
      ...
      cell_info: {
        ...cellInfo,
        capacity_mAh: Number(cellInfo.capacity_mAh) || 0,
      },
      experiment_info: {
        ...experimentInfo,
        total_cycles: experimentInfo.total_cycles,
      },
    ...
    try {
      const res = await fetch(
        "http://localhost:5678/webhook-test/start-experiment",
        {
          method: "POST",
      ...
      setMessages((prev) => [
        ...
        { id: uuidv4(), role: "user", content: text },
        {
          ...
          content: (data.output || "No response").replace(/\n/g, "<br/>")
            .replace(/<script.*?>[\s\S]*?<\/script>/gi, ""), // naive sanitize
        },
      ]);
    } catch (err: any) {
  console.error(err);
  ...
  messageApi.success("레시피 생성 완료");
        ...
        setMessages((prev) => [
          ...prev,
          {
            ...
            role: "assistant",
            content:
              "레시피 JSON 파싱 실패: 백엔드 응답을 확인하세요.<br/>RAW: " +
              (recipeData.output || "(empty)"),
          },
        ]);
      }
    } catch (err: any) {
  ...
  const ChatView = (
    <div className="flex flex-col h-[calc(100vh-160px)]">
      ...
      {errorText && (
        <Alert type="error" showIcon message={errorText} className="mb-3" />
      )}
              ...
              <div className="space-y-4">
                {messages.map((m) => (
                  <div key={m.id} className="w-full">
                    {m.role === "user" ? (
                      <div className="max-w-[70%] bg-blue-50 border border-blue-200 rounded-xl p-3 shadow-sm">
                      ...
                      <div className="ml-auto max-w-[80%] bg-green-50 border border-green-200 rounded-xl p-3 shadow-sm">
      ...
      <Card title="셀 기준정보" size="small">
        <Form layout="vertical">
          <Form.Item label="셀 ID">
            <Input
              value={cellInfo.cell_id}
              onChange={(e) => setCellInfo((s) => ({ ...s, cell_id: e.target.value }))}
              placeholder="셀 ID 입력"
            />
          </Form.Item>
          ...
          <Form.Item label="용량 (mAh)">
            <InputNumber
              className="w-full"
              value={cellInfo.capacity_mAh}
              onChange={(v) => setCellInfo((s) => ({ ...s, capacity_mAh: v ?? undefined }))}
              placeholder="예: 3000"
            />
          </Form.Item>
          <Form.Item label="제조사">
            <Input
              value={cellInfo.manufacturer}
              onChange={(e) => setCellInfo((s) => ({ ...s, manufacturer: e.target.value }))}
              placeholder="제조사 입력"
            />
          </Form.Item>
          <Form.Item label="형태">
            <Select
              ...
              options={[
                ...
                { value: "cylindrical", label: "원통형" },
                { value: "prismatic", label: "각형" },
              ]}
            />
          </Form.Item>
          <Collapse
        ...
        <Form layout="vertical">
          <Form.Item label="목표">
            <Input
              ...
              onChange={(e) => setExperimentInfo((s) => ({ ...s, objective: e.target.value }))}
              placeholder="예: 열화 유도"
            />
          </Form.Item>
          <Form.Item label="SOC 범위">
            <Slider
              ...
              max={100}
              step={1}
              value={experimentInfo.soc_range}
              onChange={(v) => setExperimentInfo((s) => ({ ...s, soc_range: v as [number, number] }))}
            />
          </Form.Item>
          <Form.Item label="총 사이클">
            <InputNumber
              className="w-full"
              value={experimentInfo.total_cycles}
              onChange={(v) =>
                setExperimentInfo((s) => ({ ...s, total_cycles: v ?? undefined, scheduling: { ...s.scheduling, total_cycles: v ?? undefined } }))
              }
              placeholder="예: 500"
            />
          </Form.Item>
          ...
          <div className="grid grid-cols-3 gap-2">
            <InputNumber
              className="w-full"
              value={experimentInfo.safety_constraints.max_voltage}
              onChange={(v) =>
                setExperimentInfo((s) => ({ ...s, safety_constraints: { ...s.safety_constraints, max_voltage: v ?? undefined } }))
              }
              placeholder="최대 전압 (예: 4.2)"
            />
            <InputNumber
              className="w-full"
              value={experimentInfo.safety_constraints.min_voltage}
              onChange={(v) =>
                setExperimentInfo((s) => ({ ...s, safety_constraints: { ...s.safety_constraints, min_voltage: v ?? undefined } }))
              }
              placeholder="최소 전압 (예: 2.5)"
            />
            <InputNumber
              className="w-full"
              value={experimentInfo.safety_constraints.max_temp}
              onChange={(v) =>
                setExperimentInfo((s) => ({ ...s, safety_constraints: { ...s.safety_constraints, max_temp: v ?? undefined } }))
              }
              placeholder="최대 온도 (예: 55)"
            />
            <InputNumber
              className="w-full"
              value={experimentInfo.safety_constraints.max_charge_c}
              onChange={(v) =>
                setExperimentInfo((s) => ({ ...s, safety_constraints: { ...s.safety_constraints, max_charge_c: v ?? undefined } }))
              }
              placeholder="최대 충전 C"
            />
            <InputNumber
              className="w-full"
              value={experimentInfo.safety_constraints.max_discharge_c}
              onChange={(v) =>
                setExperimentInfo((s) => ({ ...s, safety_constraints: { ...s.safety_constraints, max_discharge_c: v ?? undefined } }))
              }
              placeholder="최대 방전 C"
            />
            <InputNumber
              className="w-full"
              value={experimentInfo.safety_constraints.cutoff_current_c}
              onChange={(v) =>
                setExperimentInfo((s) => ({ ...s, safety_constraints: { ...s.safety_constraints, cutoff_current_c: v ?? undefined } }))
              }
              placeholder="컷오프 전류(C)"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <InputNumber
              className="w-full"
              value={experimentInfo.scheduling.rest_after_charge_min}
              onChange={(v) =>
                setExperimentInfo((s) => ({ ...s, scheduling: { ...s.scheduling, rest_after_charge_min: v ?? undefined } }))
              }
              placeholder="충전 후 휴지(min)"
            />
            <InputNumber
              className="w-full"
              value={experimentInfo.scheduling.rest_after_discharge_min}
              onChange={(v) =>
                setExperimentInfo((s) => ({ ...s, scheduling: { ...s.scheduling, rest_after_discharge_min: v ?? undefined } }))
              }
              placeholder="방전 후 휴지(min)"
            />
            <InputNumber
              className="w-full"
              value={experimentInfo.scheduling.measure_ir_every_n_cycles}
              onChange={(v) =>
                setExperimentInfo((s) => ({ ...s, scheduling: { ...s.scheduling, measure_ir_every_n_cycles: v ?? undefined } }))
              }
              placeholder="IR 측정 주기(cycles)"
            />
            <InputNumber
              className="w-full"
              value={experimentInfo.scheduling.record_timeseries_every_n_sec}
              onChange={(v) =>
                setExperimentInfo((s) => ({ ...s, scheduling: { ...s.scheduling, record_timeseries_every_n_sec: v ?? undefined } }))
              }
              placeholder="타임시리즈 주기(sec)"
            />
            <InputNumber
              className="w-full"
              value={experimentInfo.scheduling.dq_dv_every_n_cycles}
              onChange={(v) =>
                setExperimentInfo((s) => ({ ...s, scheduling: { ...s.scheduling, dq_dv_every_n_cycles: v ?? undefined } }))
              }
              placeholder="dQ/dV 주기(cycles)"
            />
            <InputNumber
              className="w-full"
              value={experimentInfo.scheduling.capacity_check_every_n_cycles}
              onChange={(v) =>
                setExperimentInfo((s) => ({ ...s, scheduling: { ...s.scheduling, capacity_check_every_n_cycles: v ?? undefined } }))
              }
              placeholder="capacity check 주기"
            />
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            <Select
              value={experimentInfo.thermal.chamber_control}
              onChange={(v) =>
                setExperimentInfo((s) => ({ ...s, thermal: { ...s.thermal, chamber_control: v } }))
              }
              options={[
                { value: "ambient", label: "주변" },
                { value: "peltier", label: "펠티어" },
                { value: "air_cooled", label: "공냉" },
                { value: "liquid_cooled", label: "수냉" },
                { value: "chamber", label: "챔버" },
                { value: "unknown", label: "미정" },
              ]}
            />
            <InputNumber
              className="w-full"
              value={experimentInfo.thermal.setpoint_c}
              onChange={(v) => setExperimentInfo((s) => ({ ...s, thermal: { ...s.thermal, setpoint_c: v ?? undefined } }))}
              placeholder="설정온도(°C)"
            />
            <InputNumber
              className="w-full"
              value={experimentInfo.thermal.tolerance_c}
              onChange={(v) => setExperimentInfo((s) => ({ ...s, thermal: { ...s.thermal, tolerance_c: v ?? undefined } }))}
              placeholder="허용편차(°C)"
            />
          </div>
      ...
      <div className="h-screen w-full bg-gray-50">
        ...
        {isGenerating && (
          <div className="fixed inset-0 pointer-events-none">
            <div className="absolute right-4 bottom-4 bg-white/90 rounded-xl shadow-lg px-3 py-2 flex items-center gap-2">
              <Spin size="small" />
              <Text>생성 중…</Text>
            </div>
          </div>
