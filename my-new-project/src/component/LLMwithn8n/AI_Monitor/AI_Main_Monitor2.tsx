...
function pickChartUrl(links: { title?: string; url: string }[]): string | null {
  const qc = links.find((l) => /quickchart\.io\/chart/i.test(l.url));
  if (qc) return qc.url;
  const img = links.find((l) => /\.(png|jpe?g|gif|svg|webp)(\?.*)?$/i.test(l.url));
  if (img) return img.url;
  const staticGraph = links.find((l) => /\/static\/graphs\/|\/chart\/|\/plot\//i.test(l.url));
  if (staticGraph) return staticGraph.url;
...
const Bubble: React.FC<{
  ...
  <div className={`flex ${role === "assistant" ? "justify-end" : "justify-start"}`}>
    ...
    >
      ...
      {/* assistant에만 우상단 PDF 아이콘 */}
      {role === "assistant" && onExport && (
        <button
          onClick={onExport}
  ...
  return (
    <div className={containerClass}>
      <div className="w-full min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
        {/* Header */}
        <div className="sticky top-0 z-10 border-b bg-white/80 backdrop-blur dark:bg-slate-900/70">
        ...
        {/* Body Grid */}
        <div className="w-full px-6 py-5 grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_420px] gap-5">
          {/* Left: Chat + Steps */}
              ...
              <CardContent className="p-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-2">
                  {STEPS.map((s, i) => {
                    ...
                    return <StepBadge key={s.key} active={active} done={done} label={s.label} desc={s.desc} icon={s.icon}/>;
                  })}
                </div>
              </CardContent>
            </Card>
          ...
          <div className="flex flex-col gap-5 min-w-0">
              ...
              <CardContent className="space-y-3 text-sm">
                {messages.length ? (
                  <div className="space-y-2">
                    {/* Lightweight auto-summary from last assistant message */}
                    {(() => {
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {(() => {
                        ...
                        return kp.slice(0,6).map((t, i) => <Badge key={i} variant="outline" className="bg-slate-100 dark:bg-slate-800">{t}</Badge>);
                      })()}
                    </div>
                  </div>
                ) : (
                    ...
                    <ul className="list-disc pl-5 space-y-1">
                      {links.map((l, i) => (
                        <li key={i} className="truncate">
                          <a href={l.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 underline inline-flex items-center gap-1">
            ...
            <Card className="border-slate-200 dark:border-slate-800">
              ...
              <CardContent>
                <div className="aspect-[16/9] w-full rounded-lg border bg-white dark:bg-slate-900 overflow-hidden grid place-items-center">
                  {chartUrl ? (
                    <img
                      src={chartUrl}
                      alt="분석 그래프"
                    ...
                    <div className="text-slate-400 text-sm">그래프가 여기에 표시됩니다.<br></br> QuickChart 전체 URL을 반환하도록 서버를 확인하세요.</div>
                  )}
                </div>
              </CardContent>
            </Card>
